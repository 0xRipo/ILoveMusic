...
- **Every API caller brings their own Spotify Developer App credentials** (`PUT /v1/spotify-credentials`, encrypted at rest, AES-256-GCM). The platform holds no shared Spotify app. This exists because Spotify now requires the app owner to have an active Premium subscription and caps Development Mode apps — a shared platform credential would either need Extended Quota approval (250k MAU minimum) or hit that cap almost immediately. The desktop app still uses its own local `.env` credentials directly (single user, no BYOK needed there).
